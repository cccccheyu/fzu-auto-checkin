...
- 💓 **（可选，默认关闭）** 每晚一条「完成确认」：勾上后，**程序没签到时**（你自己已经签过了、或今日没有晚点名）也会推一次，让你确认程序还活着。**默认不发**——代价是：程序万一没跑起来（电脑关机、脚本崩了、计划任务被删），这类故障**发不出任何通知**，你不会收到消息。签到失败 / 不在范围 / 坐标异常等**照样推送**，不受这个开关管。（云端部署勾了也没用；想打开：已装过的在 `config.yaml` 的 `notify:` 下加一行 `daily_confirm: true`，新安装的在配置页勾选即可）
...
**请在本人确实在校时使用**。长期离校（实习、交换等）请走辅导员报备请假。
