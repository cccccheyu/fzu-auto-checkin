...
在意准点、或者完全不想碰 GitHub：打开[配置页](https://cccccheyu.github.io/fzu-auto-checkin/)填完 1~3 步，在「方式 B」里下载 `setup.bat`，**双击一次全装好**——脚本会自动检查 Python（没装会给出国内镜像地址）、下载项目代码、写入配置、装依赖、创建每天 21:35 / 21:50 / 22:05 / 23:40 的计划任务，最后试运行一次（只验证，不真签）。
