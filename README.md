...
- 💓 每晚一条「完成确认」：**程序没签到时**（你自己已经签过了、或今日没有晚点名）会推一次，让你确认程序还活着——否则它哪天悄悄挂了，你只会觉得「今晚怎么没消息」。签到成功 / 失败本身另有通知，**不会重复推**。（云端部署不推送。不想收到：**已经装过的**直接在 `config.yaml` 的 `notify:` 下加一行 `daily_confirm: false`；新安装的在配置页取消勾选即可）
